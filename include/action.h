...
public:
    ...
    optional_return_t operator()(Args&&... args) const {
        ...
        if constexpr (!std::is_void_v<action_return_t>) {
            // check if return type is correct.
            static_assert(std::disjunction_v<std::is_same<action_return_t, ReturnType>,
                                             std::is_same<action_return_t, optional_return_t>>,
