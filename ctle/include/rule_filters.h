...
/**
 * @brief filters rules for the "initial" state of lexer AKA rules valid in initial states and rules
 * from non-exclsive states.
 *
 * @tparam StateDefinitions a list of all state definitions (we need to know whether they are
 * exclusive).
 */
...
class initial_filter
{
    ...
    static constexpr bool decide(ctll::list<StateDefinition...>) {
        return (
          (!StateDefinition::exclusive() && Rule::is_valid_in_state(StateDefinition::identifier()))
          || ... || Rule::is_valid_in_state(state_initial));
    }
    /**
...
public:
    /**
     * @brief filters rules for the "initial" state of lexer AKA rules valid in initial states and
     * rules from non-exclsive states.
     *
