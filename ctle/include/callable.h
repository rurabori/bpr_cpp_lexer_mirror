 ...
 * lambdas). Meaning operator() is specified.
 *
...
public:
    ...
    auto operator()(Args&&... args) const {
        static_assert(!empty(), "An empty callable has been called.");
        return m_callable(std::forward<Args>(args)...);
    }
};
// deduction guidelines.
template<typename Ty>
callable(Ty)->callable<Ty>;
template<typename Ty>
callable(const callable<Ty>&)->callable<Ty>;
