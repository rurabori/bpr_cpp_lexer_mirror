...
namespace detail {
    ...
    constexpr auto operator+(std::pair<ctll::list<Results...>, ctll::list<Types...>> input,
                             ctll::list<Type>) {
        // if no more types to judge, return what we have.
        if constexpr (!sizeof...(Types)) return input;
        ...
        if constexpr (std::is_same_v<front, Type>) {
            ...
            return std::pair<ctll::list<Results..., Type>,
                             decltype(detail::remove_type<Type, Types...>())>{};
        } else {
            // else return what we had, because the type at the front will come in some next
            // iteration.
            return input;
        }
    }
...
using unique_from_list_to_type_t
  = list_to_type_t<OutputType, decltype(detail::remove_duplicates(std::declval<List>()))>;
} // namespace ctle
